...
const articleId = route.params.articlid
...
const onSubmit = ()=>{
    save().then(x=>router.push("/blog/"+user.username))
}
const saveBtn = ()=>{
    save().then(x=>router.push("/blog/"+user.username))
}
const save = ()=>{
    ...
    if(currentSelect.value==""||data.value.articleContent==""||data.value.summary==""||data.value.title==""){
        ...
        data.value.userId = classification[0].userId
        data.value.classificationId = classification[0].id
        data.value.tags = data.value.tagsString.split(" ")
        data.value.releaseTime = (new Date()).valueOf()+""
        http.post("/article/add",data.value).then((res:any)=>{
            if(res.data.code=="0"){
                ElMessage({
                    type:"success",
                    message:"保存成功"
...
<div class="content">
    ...
    <div class="input"><div class="left">标签：</div><el-input placeholder="每个标签请用空格隔开" v-model="data.tagsString" autosize type="textarea" clearable @change=""></el-input>
    </div>
    <div class="input"><div class="left">分类：</div>
        <el-select v-model="currentSelect" @change="change" placeholder="请选择分类">
            <el-option
